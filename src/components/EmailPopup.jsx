...
// List-focused copy — no discount offer
const COPY = {
  en: {
    eyebrow:       "The List",
    headlineLine1: "For the ones",
    headlineLine2: "who entertain well.",
    subline:       "Seasonal cocktail recipes, hosting ideas, and early access to new drops — straight to your inbox.",
  ...
  fr: {
    ...
    subline:       "Recettes de saison, idées pour recevoir et accès anticipé aux nouvelles créations — directement dans votre boîte de réception.",
    consent:       "Pas de spam. Désabonnez-vous à tout moment.",
    success:       "Vous êtes sur la liste. Vérifiez votre boîte de réception.",
    placeholder:   "votre@email.com",
    claim:         "Rejoindre la liste",
...
export default function EmailPopup() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/api/subscribe`, {
        ...
        body:    JSON.stringify({ email: trimmed, language, source: "inline" }),
      })
      if (res.ok) {
        setStatus("success")
        markSubscribed()
        ...
        } catch { setErrorDetail(`Request failed (${res.status})`) }
      }
    } catch (err) {
      setStatus("error")
  ...
  const Content = (
    <div style={{ display: "flex", flexDirection: "column", flex: 1, padding: isMobile ? "28px 24px 32px" : "44px 44px 44px 48px", justifyContent: "center" }}>
      ...
      {/* Headline */}
      <h2 style={{ fontFamily: C.serif, fontSize: isMobile ? "clamp(1.7rem,7vw,2.2rem)" : "clamp(1.9rem,2.5vw,2.6rem)", fontWeight: 300, fontStyle: "italic", lineHeight: 1.1, color: C.dark, margin: "0 0 12px" }}>
        {copy.headlineLine1}<br />{copy.headlineLine2}
      </h2>

      {/* Subline */}
      <p style={{ fontFamily: C.sans, fontSize: "0.75rem", fontWeight: 300, lineHeight: 1.7, color: "rgba(15,10,4,0.45)", margin: "0 0 24px" }}>
        {copy.subline}
      </p>
        ...
        <>
          <div style={{ display: "flex", width: "100%", border: `1px solid ${C.border}`, borderRadius: "100px", background: "#fff", marginBottom: "10px" }}>
            <input
              ...
              autoComplete="email"
              style={{ flex: 1, background: "transparent", outline: "none", padding: "11px 16px", fontFamily: C.sans, fontSize: "0.78rem", fontWeight: 300, color: C.dark, minWidth: 0 }}
            />
            <button
              ...
              style={{
                ...
                borderRadius: "100px", margin: "3px", padding: "9px 16px", flexShrink: 0,
                ...
                whiteSpace: "nowrap", opacity: status === "loading" ? 0.7 : 1, transition: "opacity 0.2s",
              }}
            >
          ...
          {status === "error" && (
            <>
              <p style={{ fontFamily: C.sans, fontSize: "0.65rem", color: "rgba(200,60,60,0.75)", margin: "0 0 4px 4px" }}>{copy.error}</p>
              {errorDetail && <p style={{ fontFamily: C.sans, fontSize: "0.58rem", color: "rgba(200,60,60,0.9)", margin: "0 0 8px 4px" }}>{errorDetail}</p>}
            </>
          )}
  ...
  if (isMobile) {
    return (
      <>
        <style>{`@keyframes ppSheetUp { from { transform: translateY(100%); } to { transform: translateY(0); } }`}</style>
        <div onClick={handleDismiss} style={{ position: "fixed", inset: 0, zIndex: 50, background: "rgba(15,10,4,0.5)", backdropFilter: "blur(3px)" }} />
        <div role="dialog" aria-modal="true" aria-label={copy.headlineLine1} style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 51, background: C.cream, borderRadius: "20px 20px 0 0", boxShadow: "0 -8px 48px rgba(15,10,4,0.2)", animation: "ppSheetUp 0.38s cubic-bezier(0.22,1,0.36,1)", maxHeight: "90vh", overflowY: "auto" }}>
          <div style={{ display: "flex", justifyContent: "center", padding: "12px 0 0" }}>
            <div style={{ width: "36px", height: "4px", borderRadius: "100px", background: "rgba(15,10,4,0.12)" }} />
          </div>
          <button onClick={handleDismiss} aria-label="Close" style={{ position: "absolute", top: "16px", right: "16px", width: "28px", height: "28px", borderRadius: "50%", background: "rgba(15,10,4,0.06)", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <svg width="10" height="10" viewBox="0 0 16 16" fill="none" stroke="rgba(15,10,4,0.45)" strokeWidth="1.5" strokeLinecap="round"><path d="M2 2l12 12M14 2L2 14" /></svg>
          </button>
          {Content}
        </div>
      </>
  ...
  return (
    <>
      <style>{`@keyframes ppModalIn { from { opacity: 0; transform: translate(-50%, -47%) scale(0.96); } to { opacity: 1; transform: translate(-50%, -50%) scale(1); } }`}</style>
      <div onClick={handleDismiss} style={{ position: "fixed", inset: 0, zIndex: 50, background: "rgba(15,10,4,0.55)", backdropFilter: "blur(4px)" }} />
      <div role="dialog" aria-modal="true" aria-label={copy.headlineLine1} style={{ position: "fixed", top: "50%", left: "50%", zIndex: 51, transform: "translate(-50%, -50%)", width: "min(92vw, 780px)", display: "flex", overflow: "hidden", borderRadius: "20px", boxShadow: "0 32px 80px rgba(15,10,4,0.38)", animation: "ppModalIn 0.38s cubic-bezier(0.22,1,0.36,1)" }}>
        <div style={{ flex: 1, background: C.cream, minWidth: 0 }}>{Content}</div>
        <div style={{ width: "280px", flexShrink: 0, position: "relative" }}>
          <img src="/images/driedcitrusbanner.jpg" alt={copy.imageAlt} style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover", objectPosition: "center" }} />
          <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to right, rgba(250,247,242,0.2) 0%, transparent 35%)" }} />
        </div>
        <button onClick={handleDismiss} aria-label="Close" style={{ position: "absolute", top: "12px", right: "12px", width: "28px", height: "28px", borderRadius: "50%", background: "rgba(250,247,242,0.88)", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", transition: "background 0.15s" }} onMouseEnter={e => e.currentTarget.style.background = "#fff"} onMouseLeave={e => e.currentTarget.style.background = "rgba(250,247,242,0.88)"}>
          <svg width="10" height="10" viewBox="0 0 16 16" fill="none" stroke="rgba(15,10,4,0.5)" strokeWidth="1.5" strokeLinecap="round"><path d="M2 2l12 12M14 2L2 14" /></svg>
        </button>
      </div>
    </>
  )
