...
app.post('/api/webhook', webhookLimiter, express.raw({ type: 'application/json' }), async (req, res) => {
  ...
  switch (event.type) {
    ...
    case 'payment_intent.succeeded': {
      // Do NOT create orders here for Stripe Checkout. This event fires before
      // checkout.session.completed and has incomplete customer/shipping; it produced duplicate
      // orders, N/A emails, and failed Chit Chats calls. Orders come from:
      //   checkout.session.completed (webhook) and GET /api/checkout-session/:id (success page).
      const paymentIntent = event.data.object
      console.log('✅ PaymentIntent succeeded:', paymentIntent.id, '(order handled via Checkout Session webhook or /api/checkout-session)')
      break
    }
...
app.post('/api/subscribe', apiLimiter, async (req, res) => {
  ...
  try {
    ...
    console.log('Subscribe: sending welcome email to', email, '| template: list')
